        ...
        <thead>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <img src="{{$pelicula->imagen}}" width="60%">
                        </td>
                    <td>{{$pelicula->stock}}</td>
                    <td>{{$pelicula->precio_al}}</td>
                    <td>{{$pelicula->precio_com}}</td>
                    <td>{{$pelicula->disponivilidad}}</td>
                    <td>{{$pelicula->likes}}</td>
                    <td>

                        <form action="{{url('peliculas/'.$pelicula->id.'/quitar')}}" method='get'>
                            @csrf
                            @method('PUT')
                    ...
                    <form action="{{ route('peliculas.destroy', $pelicula->id)}}" method="POST">
                        <a href="peliculas/{{$pelicula->id}}/edit" class="btn btn-info text-white my-2" style="width: 200px;">Editar</a>
                        @csrf
                        @method('DELETE')
