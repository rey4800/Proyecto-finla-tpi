...
<style>
        ...
        <style>
            body {
                font-family: 'Nunito', sans-serif;/*tipo de letra a  utlizar en  el navbar */
                background-image: linear-gradient(35deg,#000000,#2b2e53,#2F3F80,#161569,#5A4894,#A950A1,#FF7CAE);
                background-size: 500%;/*tamanio del fondo */
                min-height: 100vh;
            }
            .navbar:hover {/*cuando el mouse se acerca se  pone  de  color negro */
    ...
    <div class="navbar  sticky-top navbar-light"  style="width:100%; height:70px;display:flex;">
      ...
      <div class="px-4 " style="float: right;">
        <nav class="text-white px-3">
           <!--Condicion que ayuda a identificar si esta logueado el usuario-->
        @if(auth()->check())
         
            <span><a class="a text-sm text-red underline">Bienvenido <b>{{auth()->user()->name}}</b></a></span>
            @if(auth()->user()->role == 'admin') <!--Condicion que ayuda a identificar  que tipo de rol es el usuario-->
          ...
          @else <!--si el  usuario no esta  registrado le saldra en  el navbar esto-->
